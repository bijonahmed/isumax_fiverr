...
class PaypalController extends Controller
{
    ...
    public function __construct(PayPalClient $provider)
    {
        ...
        if (!empty($id)) {
            ...
            $this->return_website =  env('FRONTEND_URL', 'https://isumax.com');
        }
    }
    ...
    {
        // Capture the token (order ID) from the query string
        ...
        if (isset($response['status']) && $response['status'] == 'COMPLETED') {
            return response()->json([
                ...
                'redirect_url' => $this->return_website . '/paypalsuccess?token=' . $token,
            ]);
        } else {
            return response()->json([
                'status' => 'error',
