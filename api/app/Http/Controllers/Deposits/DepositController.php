...
class DepositController extends Controller
{
    ...
    public function sendRequesPaypalPayment(Request $request)
    {
        //dd($request->all());
        try {
            $validator = Validator::make($request->all(), [
    ...
    public function getUserDepositList(Request $request)
    {
        $userId     = $this->userid;
        $data       = Deposit::where('user_id', $userId)
            ->select(
                'depositID',
                'currency',
                DB::raw('FORMAT(deposit_amount, 2) as deposit_amount'),  // Format deposit_amount with 2 decimal places
                'payment_method',
                'payment_status',
        ...
        // Calculate the total deposit_amount where payment_status = 1
        $totalDepositAmount = Deposit::where('user_id', $userId)
                            ->where('status', 1)
                            ->where('payment_status', 'succeeded')
                            ->whereIn('payment_method', ['Stripe', 'Paypal'])
                            ->sum('deposit_amount');  // Sum the deposit_amount


        return response()->json([
            'data'          => $data,
            'trans_count'   => count($data),
            'total_deposit' => number_format($totalDepositAmount, 2),  // Format the total deposit amount to 2 decimal places
